...
describe('POST /incomes', () => {
  test('crea un ingreso exitosamente', async () => {
    ...
    expect(response.body).toHaveProperty('merchant', incomeData.merchant);
    expect(response.body).toHaveProperty('amount', incomeData.amount);
    expect(response.body).toHaveProperty('category', incomeData.category);
    expect(response.body).toHaveProperty('date', incomeData.date);
...
describe('GET /incomes', () => {
  test('lista todos los ingresos del usuario autenticado', async () => {
    const userId = await createTestUser('test@example.com', 'password123', 'Test User');
    const token = await getAuthToken('test@example.com', 'password123');
    ...
    await pool.query(
      'INSERT INTO incomes (user_id, merchant, amount, category, date) VALUES ($1, $2, $3, $4, $5)',
      [userId, 'Freelance', 500.00, 'Trabajo', '2024-01-16']
    );
  ...
  test('filtra ingresos por año', async () => {
    const userId = await createTestUser('test@example.com', 'password123', 'Test User');
    const token = await getAuthToken('test@example.com', 'password123');
    ...
    await pool.query(
      'INSERT INTO incomes (user_id, merchant, amount, category, date) VALUES ($1, $2, $3, $4, $5)',
      [userId, 'Freelance', 500.00, 'Trabajo', '2023-12-31']
    );

    const response = await request(app)
      .get('/incomes?year=2024')
      .set('Authorization', `Bearer ${token}`)
      .expect(200);

    expect(response.body).toHaveLength(1);
    expect(response.body[0].merchant).toBe('Salario');
  });

  test('filtra ingresos por año y mes', async () => {
    const userId = await createTestUser('test@example.com', 'password123', 'Test User');
    const token = await getAuthToken('test@example.com', 'password123');
    ...
    await pool.query(
      'INSERT INTO incomes (user_id, merchant, amount, category, date) VALUES ($1, $2, $3, $4, $5)',
      [userId, 'Freelance', 500.00, 'Trabajo', '2024-02-15']
    );

    const response = await request(app)
      .get('/incomes?year=2024&month=1')
      .set('Authorization', `Bearer ${token}`)
      .expect(200);
